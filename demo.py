...
def funding():
    ...
    demo_url = request.form['url']
    if not validators.url(demo_url):
    ...
    if response['status'] != 'OK':
        return render_template('index.html', text = 'Error establishing connection')
...
#Last resort
        ...
        # If by now we still don't know the investors:
    ...
    # response = alchemyapi.relations('text', sentence_score['sentence'])
    # return (json.dumps(funding_info,indent=4))
    return render_template('index.html', data = funding_info)
    # if response['status'] == 'OK':
        # print('## Object ##')
        # print(json.dumps(response, indent=4))
        ...
        # if re.findall(r"(?:secure|raise)",relation['action']['text']):
        #     if relation['action']['verb']['tense'] is not "future":
        #         # print (relation['subject']['text'])
        #         # print (relation['action']['text'])
        #         # print (relation['object']['text'])
        #         subject = alchemyapi.entities('text', relation['subject']['text'])
        #         for entity in subject['entities']:
        #             if entity['type'] == 'Company':
        #                 startup = entity['text']
        #         print (startup)

        #         object = alchemyapi.entities('text', relation['object']['text'])
        #         print (json.dumps(object['entities'],indent=4))
                # for entity in object['entities']:
                    # if entity['type'] == 'Company':
                        # startup = entity['text']
                # print (startup)

            # matches = re.findall(r"(^.*?\b%s\b.*\b(?:secure|raise).*?\b%s\b.*?$)" % (pair[0]['text'],pair[1]['text']), response['text'], re.M)
